...
class RComisionistasNaturalesPDF extends  ReportePDF {
    var $datos_titulo;
    var $datos_detalle;
    var $ancho_hoja;
    var $gerencia;
    ...
    function Header() {
        ...
        $this->ln(7);
        $this->SetFont('', '', 10);
        $this->Cell(0, 5, "PERSONAS NATURALES", 0, 1, 'C');
        ...
        $cabecera = array(15,16,14,50,53,25,45,20,20);
           //$array = array(15,16,14,55,30,40,20,28,25);
        $this->ln(6);
        $this->Cell($cabecera[0],3,'Cod. Cliente','',0,'C');
        $this->Cell($cabecera[1],3,'Tipo de Doc.','',0,'C');
        $this->Cell($cabecera[2],3,'Nro. Doc','',0,'C');
        $this->Cell($cabecera[3],3,'Cliente','',0,'C');
        $this->Cell($cabecera[4],3,'Cod. Producto','',0,'C');
        $this->Cell($cabecera[5],3,'Desc. Producto','',0,'C');
        $this->Cell($cabecera[6],3,'Cant. Producto','',0,'C');
        $this->Cell($cabecera[7],3,'P/U (Bs)','',0,'L');
        $this->Cell($cabecera[8],3,'Total (Bs)','',0,'C');
        $this->Ln();
    ...
    function generarReporte(){
      ...
      //$this->SetAutoPageBreak(true,2);
        ...
        $this->SetFont('', '', 7);
        $array = array(15,16,14,65,30,40,20,28,25);

        $this->SetFont('', '', 7);
        foreach ($this->datos as $key => $value) {

                $this->Cell($array[0],3,$value['nit'],'',0,'C');
                $this->Cell($array[1],3,$value['carnet_ide'],'',0,'C');
                $this->Cell($array[2],3,$value['nit'],'',0,'C');
                $this->Cell($array[3],3,$value['razon_social'],'',0,'L');
                $this->Cell($array[4],3,$value['sistema_origen'],'',0,'L');
                $this->Cell($array[5],3,$value['desc_ruta'],'',0,'L');
                $this->Cell($array[6],3,$value['cantidad'],'',0,'R');
                $this->Cell($array[7],3,number_format($value['precio_unitario'], 2, ',', '.'),'',0,'R');
                $this->Cell($array[8],3,number_format($value['precio_total'], 2, ',', '.'),'',0,'R');
                $this->Ln();
