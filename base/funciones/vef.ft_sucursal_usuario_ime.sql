...
BEGIN
	...
	***********************************/
	...
	if(p_transaccion='VF_SUCUSU_INS')then
        ...
        begin
				-- ini breydi.vasquez, fecha: 25/06/2021, desc: control duplicidad por punto de venta
					if exists (select 1 from vef.tsucursal_usuario
							where id_usuario = v_parametros.id_usuario and id_punto_venta = v_parametros.id_punto_venta
									 and tipo_usuario = v_parametros.tipo_usuario) then

							raise 'Estimad@: el usuario que intenta ingresar ya se encuentra registrado, para el punto de venta y con el tipo de usuario %',v_parametros.tipo_usuario;
